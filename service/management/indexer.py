...
INDEX_NAMES = [
    ...
    N.AGGLOMERATIONS,
    es_config.geom_index_for(N.AGGLOMERATIONS),
    N.CENSUS_TRACTS,
    es_config.geom_index_for(N.CENSUS_TRACTS),
    N.CENSUS_BLOCKS,
    es_config.geom_index_for(N.CENSUS_BLOCKS),
    N.LOCAL_GOVERNMENTS,
...
def run_index(es, forced, name='all', verbose=False):
    ...
    indices = [
        ...
        GeorefIndex(alias=es_config.geom_index_for(N.DEPARTMENTS),
                    ...
                    filepath=app.config['DEPARTMENTS_FILE'],
                    includes=[N.ID, N.GEOM]),
        GeorefIndex(alias=N.AGGLOMERATIONS,
                    doc_class=es_config.Agglomeration,
                    filepath=app.config['AGGLOMERATIONS_FILE'],
                    synonyms_filepath=app.config['SYNONYMS_FILE'],
                    excluding_terms_filepath=app.config[
                        'EXCLUDING_TERMS_FILE'],
                    backup_filepath=os.path.join(backups_dir,
                                                 'aglomerados.ndjson')),
        GeorefIndex(alias=es_config.geom_index_for(N.AGGLOMERATIONS),
                    doc_class=es_config.AgglomerationGeom,
                    filepath=app.config['AGGLOMERATIONS_FILE'],
                    includes=[N.ID, N.GEOM]),
        GeorefIndex(alias=N.CENSUS_TRACTS,
                    doc_class=es_config.CensusTracts,
                    filepath=app.config['CENSUS_TRACTS_FILE'],
                    synonyms_filepath=app.config['SYNONYMS_FILE'],
                    excluding_terms_filepath=app.config[
                        'EXCLUDING_TERMS_FILE'],
                    backup_filepath=os.path.join(backups_dir,
                                                 'fracciones_censales.ndjson')),
        GeorefIndex(alias=es_config.geom_index_for(N.CENSUS_TRACTS),
                    doc_class=es_config.CensusTractsGeom,
                    filepath=app.config['CENSUS_TRACTS_FILE'],
                    includes=[N.ID, N.GEOM]),
        GeorefIndex(alias=N.CENSUS_BLOCKS,
                    doc_class=es_config.CensusBlocks,
                    filepath=app.config['CENSUS_BLOCKS_FILE'],
                    synonyms_filepath=app.config['SYNONYMS_FILE'],
                    excluding_terms_filepath=app.config[
                        'EXCLUDING_TERMS_FILE'],
                    backup_filepath=os.path.join(backups_dir,
                                                 'radios_censales.ndjson')),
        GeorefIndex(alias=es_config.geom_index_for(N.CENSUS_BLOCKS),
                    doc_class=es_config.CensusBlocksGeom,
                    filepath=app.config['CENSUS_BLOCKS_FILE'],
                    includes=[N.ID, N.GEOM]),
        ...
        GeorefIndex(alias=N.UNIVERSITY_INSTITUTIONS,
                    doc_class=es_config.UniversityInstitutions,
                    filepath=app.config['UNIVERSITY_INSTITUTIONS_FILE'],
                    synonyms_filepath=app.config['SYNONYMS_FILE'],
                    excluding_terms_filepath=app.config[
                        'EXCLUDING_TERMS_FILE'],
                    backup_filepath=os.path.join(backups_dir,
                                                 'instituciones_universitarias.ndjson')),
    ]
