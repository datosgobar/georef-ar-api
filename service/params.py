...
class IntersectionParameter(Parameter):
    ...
    def __init__(self, entities, required=False):
        ...
        for entity in entities:
            length_tuple = (id_lengths[entity],) if isinstance(id_lengths[entity], int) else id_lengths[entity]
            self._id_params[entity] = IdsFixedLengthParameter(
                *length_tuple, sep=':')
...
PARAMS_CENSUS_BLOCKS = EndpointParameters(shared_params={
    ...
    N.FIELDS: FieldListParameter(basic=[N.ID],
                                 standard=[N.C_LAT, N.C_LON,
                                           N.STATE_ID, N.STATE_NAME, N.DEPT_ID, N.DEPT_NAME,
                                           ],
...
PARAMS_NEARBY_ESTABLISHMENTS = EndpointParameters(shared_params={
    ...
    N.FORMAT: StrParameter(default='json', choices=['json', 'geojson', 'xml', 'csv', 'shp'])
})
...
PARAMS_EDUCATIONAL_INSTITUTIONS = EndpointParameters(shared_params={
    N.ID: IdsParameter(id_length=constants.EDUCATIONAL_INSTITUTIONS_ID_LEN),
    N.NAME: StrParameter(),
    N.CATEGORY: StrParameter(),
    N.DEPT: CompoundParameter([IdsParameter(constants.DEPT_ID_LEN),
                               StrParameter()]),
...
PARAMS_UNIVERSITY_INSTITUTIONS = EndpointParameters(shared_params={
    N.ID: IdsParameter(id_length=constants.UNIVERSITY_INSTITUTIONS_ID_LEN),
    N.NAME: StrParameter(),
    N.CATEGORY: StrParameter(),
    N.DEPT: CompoundParameter([IdsParameter(constants.DEPT_ID_LEN),
                               StrParameter()]),
    ...
    N.FIELDS: FieldListParameter(basic=[N.C_LAT, N.C_LON, N.ID],
                                 ...
                                 complete=[
                                     N.ADMINISTRATION, N.SOURCE, N.LEVELS, N.ACADEMIC_UNITY, N.UNIVERSITY
                                 ]),
    N.MAX: IntParameter(default=10, lower_limit=1,
                        upper_limit=constants.MAX_RESULT_LEN),
    N.OFFSET: IntParameter(lower_limit=0,
